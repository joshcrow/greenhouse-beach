...
def _generate_joke_or_riddle_paragraph(narrative_body: str) -> str:
    ...
    prompt_lines = [
        ...
        "",
        "TONE:",
        "- Be a little cheeky. Dry wit is good. Dad-joke energy is acceptable.",
        "- Think: clever wordplay, groan-worthy puns, or absurdist humor.",
        "- Bonus points for riddles that poke fun at: tourists, beach life, plants that",
        "  won't cooperate, dogs being dogs, or the general chaos of coastal living.",
        "",
        "CRITICAL REQUIREMENTS:",
        "- You MUST write a RIDDLE (question format). Answer revealed tomorrow.",
        "- NEVER include the answer. No 'Because...' or explanations.",
        "",
        "FORMAT:",
        "- Start with INTRO text if provided, then the riddle question.",
        "- 1-2 sentences max. End with a question mark.",
        "- Example: 'What do you call a tourist who finally learns to parallel park? A local.'",
        "  (But as a riddle: 'What do you call a tourist who finally learns to parallel park?')",
        "",
        "THEMES (pick one that fits today's vibe):",
        "- Gardening mishaps, stubborn plants, greenhouse drama",
        "- Colington life, the harbour, beach weather, salt air",
        "- Dogs, tourists (fair game for gentle roasting), fishing",
        "- Weather complaints, seasons, nature being nature",
        "",
        "FORBIDDEN:",
        "- No answers or punchlines in your output.",
        "- No emojis, markdown, or HTML.",
        "",
        f"INTRO: {intro}" if intro else "INTRO: (none - start directly with riddle)",
        "",
        "TODAY'S NEWSLETTER VIBE:",
        narrative_body[:1500] if narrative_body else "A typical day at the greenhouse.",
        "",
        "OUTPUT: Return only the riddle paragraph. No answer.",
    ]
    prompt = "\n".join(prompt_lines)
