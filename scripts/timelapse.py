    ...
    try:
        ...
        if daily_list:
            ...
            if sunrise_ts and sunset_ts:
                ...
                log(f"Daylight hours for {target_date.strftime('%Y-%m-%d')}: {sunrise.strftime('%H:%M')} - {sunset.strftime('%H:%M')}")
                return sunrise, sunset
...
def create_timelapse_gif(
    ...
    log(f"Created timelapse GIF: {len(gif_bytes)} bytes ({len(gif_bytes)/1024/1024:.1f}MB)")
