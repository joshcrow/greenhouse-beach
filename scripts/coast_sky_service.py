...
def _fetch_noaa_tides(date_local: datetime) -> Dict[str, Any]:
    ...
    try:
        log(f"Fetching NOAA tides for station {NOAA_STATION_ID} ({begin_date} to {end_date})")
        ...
        is_king_tide_window = max_high_ft is not None and max_high_ft >= KING_TIDE_THRESHOLD_FT
        ...
        log(f"NOAA tides fetched: {len(high_tides)} highs, {len(low_tides)} lows, max={max_high_ft}ft")
        return tide_summary
...
def _evaluate_meteor_showers(date_local: datetime) -> Dict[str, Any]:
    ...
    for shower in calendar:
        ...
        try:
            active_start = datetime.strptime(shower.get("active_start", ""), "%m-%d").date().replace(year=today.year)
            active_end = datetime.strptime(shower.get("active_end", ""), "%m-%d").date().replace(year=today.year)
            peak_start = datetime.strptime(shower.get("peak_start", ""), "%m-%d").date().replace(year=today.year)
            peak_end = datetime.strptime(shower.get("peak_end", ""), "%m-%d").date().replace(year=today.year)
        except (ValueError, TypeError):
            continue
        ...
        is_peak_window = -3 <= days_to_peak_start <= 3 or -3 <= days_to_peak_end <= 3 or (peak_start <= today <= peak_end)
...
def _evaluate_moon_events(date_local: datetime, moon_phase: Optional[float] = None) -> Dict[str, Any]:
